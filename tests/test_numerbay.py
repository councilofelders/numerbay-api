...
def test_upload_encrypted_artifact(api, tmpdir):
    ...
    artifact_id = "abc"
    data = {"url": "https://uploadurl", "id": artifact_id}
    responses.add(responses.POST, f"{API_ENDPOINT_URL}/artifacts/generate-upload-url", json=data)
    responses.add(responses.PUT, "https://uploadurl")
    data = {"id": artifact_id}
    responses.add(responses.POST, f"{API_ENDPOINT_URL}/artifacts/{artifact_id}/validate-upload", json=data)
    ...
    responses.add(responses.POST, f"{API_ENDPOINT_URL}/products/search-authenticated", json=data)
    artifacts = api.upload_artifact(str(path), product_full_name="numerai-predictions-mymodel")
    assert artifacts[0]["id"] == artifact_id
...
def test_download_encrypted_artifact(api, tmpdir):
    ...
    data = "https://downloadurl"
    content = b'hello'
    responses.add(responses.GET, f"{API_ENDPOINT_URL}/artifacts/{artifact_id}/generate-download-url", json=data)
    responses.add(responses.GET, "https://downloadurl", body=SealedBox(key_pair.public_key).encrypt(content))

    # download file with product_id and artifact_id
    api.download_artifact(dest_path=tmpdir.join("encrypted_artifact_1.csv"), product_id=product_id, artifact_id=artifact_id,
    ...
    api.download_artifact(dest_path=tmpdir.join("encrypted_artifact_2.csv"), product_full_name="numerai-predictions-somemodel", key_base64=private_key.decode('ascii'))
